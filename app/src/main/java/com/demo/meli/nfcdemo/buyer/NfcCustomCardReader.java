 ...
 */
public class NfcCustomCardReader implements NfcAdapter.ReaderCallback {
    private static final String TAG = "NfcCustomCardReader";
    ...
    // Weak reference to prevent retain loop. mTagReadCallback is responsible for exiting
    // foreground mode before it becomes invalid (e.g. during onPause() or onStop()).
    private WeakReference<TagReadCallback> mTagReadCallback;

    public interface TagReadCallback {
        public void onTagReceived(String payload);
    }

    public NfcCustomCardReader(TagReadCallback tagReadCallback) {
        mTagReadCallback = new WeakReference<TagReadCallback>(tagReadCallback);
    }
    ...
    public void onTagDiscovered(Tag tag) {
        ...
        if (isoDep != null) {
            try {
                ...
                // Build SELECT AID command for our custom card service.
                // This command tells the remote device which service we wish to communicate with.
                Log.i(TAG, "Requesting remote AID: " + CUSTOM_CARD_AID);
                byte[] selCommand = BuildSelectApdu(CUSTOM_CARD_AID);
                ...
                // optional payload, which is used here to hold the payload.
                int resultLength = result.length;
                byte[] statusWord = {result[resultLength-2], result[resultLength-1]};
                byte[] payload = Arrays.copyOf(result, resultLength-2);
                if (Arrays.equals(SELECT_OK_SW, statusWord)) {
                    // The remote NFC device will immediately respond with its stored payload
                    String payloadValue = new String(payload, "UTF-8");
                    Log.i(TAG, "Received: " + payloadValue);
                    // Inform CardReaderFragment of received payload
                    if (true) {
                        ...
                        mTagReadCallback.get().onTagReceived(payloadValue);
